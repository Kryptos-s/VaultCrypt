...
namespace vaultcrypt {
    ...
    AESGCMCipher::AESGCMCipher() : backend_(create_cryptopp_backend()) {}
    AESGCMCipher::~AESGCMCipher() = default;

    SecureBytes AESGCMCipher::encrypt(
        const SecureBytes& key,
        const SecureBytes& plaintext,
        const SecureBytes& aad
    ) {
        if (key.size() != KEY_SIZE) {
            throw VaultCryptException(ErrorCode::InvalidKey, "Invalid key size for AES-256");
        }
        ...
        header.algorithm = static_cast<uint8_t>(Algorithm::AES_256);
    ...
    SecureBytes AESGCMCipher::decrypt(
        const SecureBytes& key,
        const SecureBytes& ciphertext,
        const SecureBytes& aad
    ) {
        std::cerr << "[DEBUG] decrypt() called with " << ciphertext.size() << " bytes\n";
        ...
        std::cerr << "[DEBUG] Nonce length: " << nonce_len << "\n";
        ...
        std::cerr << "[DEBUG] Calling backend with " << ciphertext_with_tag.size() << " bytes\n";

        SecureBytes plaintext = backend_->decrypt(
            Algorithm::AES_256,
            Mode::GCM,
            key,
            ciphertext_with_tag,
        ...
        std::cerr << "[DEBUG] Backend returned " << plaintext.size() << " bytes\n";

        return plaintext;
    }

    // PASSWORD-BASED ENCRYPTION - SIMPLIFIED FORMAT
    SecureBytes AESGCMCipher::encrypt_password(
    ...
    SecureBytes AESGCMCipher::decrypt_password(
        const SecureString& password,
        const SecureBytes& ciphertext,
        const SecureBytes& aad
    ) {
        std::cerr << "[DEBUG] decrypt_password() called with " << ciphertext.size() << " bytes\n";
        ...
        std::cerr << "[DEBUG] Salt length: " << salt_len << "\n";
        ...
        std::cerr << "[DEBUG] Offset after KDF params: " << offset << "\n";

        // Extract encrypted data
        SecureBytes encrypted(ciphertext.begin() + offset, ciphertext.end());

        std::cerr << "[DEBUG] Encrypted payload size: " << encrypted.size() << " bytes\n";
        ...
        std::cerr << "[DEBUG] Deriving key...\n";
        SecureBytes key = derive_key(password, params, KEY_SIZE);
        std::cerr << "[DEBUG] Key derived, calling decrypt()...\n";

        // Decrypt
        SecureBytes plaintext = decrypt(key, encrypted, aad);

        std::cerr << "[DEBUG] decrypt() returned " << plaintext.size() << " bytes\n";
