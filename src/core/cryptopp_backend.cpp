    ...
    public:
        SecureBytes encrypt(
            ...
            if (alg == Algorithm::AES_256 && mode == Mode::GCM) {
                ...
                SecureBytes ciphertext(plaintext.size() + 16);  // plaintext + tag
                ...
                size_t actual_size = aef.MaxRetrievable();
                ciphertext.resize(actual_size);

                std::cerr << "[BACKEND] Encrypted " << plaintext.size() << " bytes -> "
                    << ciphertext.size() << " bytes (with tag)\n";
        ...
        SecureBytes decrypt(
            ...
            if (alg == Algorithm::AES_256 && mode == Mode::GCM) {
                ...
                // Allocate output buffer (should be ciphertext size - tag size)
                SecureBytes plaintext(ciphertext.size());  // Over-allocate, will resize
                ...
                // Get actual plaintext size
                size_t plaintext_size = adf.MaxRetrievable();
